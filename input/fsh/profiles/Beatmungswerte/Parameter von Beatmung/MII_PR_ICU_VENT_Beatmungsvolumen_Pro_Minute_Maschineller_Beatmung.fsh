Profile: MII_PR_ICU_VENT_Beatmungsvolumen_Pro_Minute_Maschineller_Beatmung
Parent: MII_PR_ICU_Parameter_Von_Beatmung
Id: mii-pr-icu-vent-beatmungsvolumen-min-maschineller-beatmung
Title: "MII PR ICU Beatmungsvolumen Pro Minute Maschineller Beatmung"
* insert PR_CS_VS_Version
* insert Publisher
* ^url = "https://www.medizininformatik-initiative.de/fhir/ext/modul-icu/StructureDefinition/mii-pr-icu-vent-beatmungsvolumen-pro-minute-maschineller-beatmung"
* ^status = #active

* category contains kuenstlicheBeatmung 1..1
* category[kuenstlicheBeatmung] = $sct#40617009 "Artificial ventilation (regime/therapy)"

* code.coding 3..
* code.coding[sct] 1..1
* code.coding[sct] ^patternCoding.code = #250875001
* code.coding[sct] ^patternCoding.display = "Ventilator delivered minute volume (observable entity)"
* code.coding[loinc] 1..1
* code.coding[loinc] ^patternCoding.code = #76009-0
* code.coding[loinc] ^patternCoding.display = "Inspired minute Volume during Mechanical ventilation"
* code.coding[IEEE-11073] 1..1
* code.coding[IEEE-11073] ^patternCoding.code = #152004
* code.coding[IEEE-11073] ^patternCoding.display = "Total volume of gas breathed in during 1 min during mechanical ventilation."
* valueQuantity = $ucum#L/min