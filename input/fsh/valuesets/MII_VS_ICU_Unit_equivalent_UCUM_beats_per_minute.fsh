ValueSet: MII_VS_ICU_Unit_equivalent_UCUM_beats_per_minute
Id: mii-vs-icu-unit-equivalent-ucum-beats-per-minute
Title: "MII VS ICU Unit equivalent UCUM beats per minute"
Description: "Dieses ValueSet enthält äquivalente Codes für Herzschläge pro Minute."
* insert PR_CS_VS_Version
* insert Publisher
* ^url = "https://www.medizininformatik-initiative.de/fhir/ext/modul-icu/ValueSet/vs-mii-icu-unit-equivalent-ucum-beats-per-minute"
* ^version = "2025.0.4"
* ^status = #active
* ^date = "2025-02-11"
* ^experimental = false

* include $ucum#/min
* include $ucum#1/min