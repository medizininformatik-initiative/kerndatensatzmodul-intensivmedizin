...
| Placeholder | Where |
|---|---|
| `{{CALVER_VERSION}}` | `version.fsh`, `meta-profile.fsh`, `cps-rules.fsh` |
| `{{MODULE_SLUG}}` | `version.fsh` (package id + canonical of the package source) |
| `{{CALVER_YEAR}}` | `version.fsh` (`resource-effectivePeriod.start`) |
